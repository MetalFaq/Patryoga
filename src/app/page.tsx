...
export default function Home() {
  ...
  async function saveStudent(event: FormEvent<HTMLFormElement>, classIds: string[]) {
    ...
    try { const id = studentEditor !== "new" && studentEditor ? studentEditor.id : undefined; const response = await fetch(id ? `/api/students/${id}` : "/api/students", { method: id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...(id ? {} : { id: String(data.get("id") ?? "").trim() || undefined }), name: String(data.get("name") ?? "").trim(), phone: String(data.get("phone") ?? "").trim(), notes: String(data.get("notes") ?? "") }) }); const payload = (await response.json()) as { error?: string; student?: Student }; if (!response.ok || !payload.student) throw new Error(payload.error ?? "No se pudo guardar la alumna."); const previous = id ? classes.filter((item) => item.studentIds.includes(id)).map((item) => item.id) : []; await updateAssignments(payload.student, classIds, previous); await reloadData(); setStudentEditor(null); } catch (error) { setEditorError(errorMessage(error, "No se pudo guardar la alumna.")); } finally { setIsMutating(false); }
  }
  ...
  async function updateAssignments(student: Student, classIds: string[], previous: string[]) {
    setIsMutating(true); setEditorError(null); try { const added = classIds.filter((id) => !previous.includes(id)); const removed = previous.filter((id) => !classIds.includes(id)); for (const ids of added.length ? [added] : []) { const response = await fetch(`/api/students/${student.id}/classes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ classIds: ids }) }); if (!response.ok) throw new Error(((await response.json()) as { error?: string }).error ?? "No se pudo asignar el horario."); } for (const ids of removed.length ? [removed] : []) { const response = await fetch(`/api/students/${student.id}/classes`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ classIds: ids }) }); if (!response.ok) throw new Error(((await response.json()) as { error?: string }).error ?? "No se pudo quitar el horario."); } await reloadData(); setStudentEditor(null); } catch (error) { setEditorError(errorMessage(error, "No se pudieron guardar los horarios.")); } finally { setIsMutating(false); }
  }
